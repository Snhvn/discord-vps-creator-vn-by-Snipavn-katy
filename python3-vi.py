...
async def create_server_task_debian(interaction):
    ...
    ssh_session_line = await capture_ssh_session_line(exec_cmd)
    if ssh_session_line:
        await interaction.user.send(embed=discord.Embed(description=f"<:Himouto:1174718684590264413>Đã tạo thành công Instance\nSSH Session Command<:Himouto:1174718684590264413>: ```{ssh_session_line}```[Support Discord](https://dsc.gg/servertipacvn)\nOS:<:debian:1344300752411164682>Debian 12", color=0x00ff00))
        add_to_database(user, container_id, ssh_session_line)
...
async def create_server_task_alpine(interaction):
    ...
    ssh_session_line = await capture_ssh_session_line(exec_cmd)
    if ssh_session_line:
        await interaction.user.send(embed=discord.Embed(description=f"<:Himouto:1174718684590264413>Đã tạo thành công Instance\nSSH Session Command<:Himouto:1174718684590264413>: ```{ssh_session_line}```[Support Discord](https://dsc.gg/servertipacvn)\nOS:<:alpine:1345340462055166012>Alpine 3.19", color=0x00ff00))
        add_to_database(user, container_id, ssh_session_line)
        await interaction.followup.send(embed=discord.Embed(description="Phiên bản đã được tạo thành công. Kiểm tra DM của bạn để biết chi tiết.", color=0x00ff00))
    else:
        await interaction.followup.send(embed=discord.Embed(description="Something went wrong or the Instance is taking longer than expected. If this problem continues, Contact Support.", color=0xff0000))
...
async def help_command(interaction: discord.Interaction):
    ...
    embed.add_field(name="<:ubuntu:1344300653324927046>|<@1236584616215711835>deploy-ubuntu", value="Tạo một Instance mới với Ubuntu 22.04.", inline=False)
    embed.add_field(name="<:debian:1344300752411164682>|<@1236584616215711835>deploy-debian", value="Tạo một Instance mới với Debian 12.", inline=False)
    embed.add_field(name="<:alpine:1345340462055166012>|<@1236584616215711835>deploy-alpine", value="Tạo một Instance mới với Alpine 3.19.", inline=False)
    embed.add_field(name="<@1236584616215711835>remove <ssh_command/Name>", value="Xóa một máy chủ", inline=False)
    embed.add_field(name="<@1236584616215711835>start <ssh_command/Name>", value="Khởi động máy chủ.", inline=False)
    embed.add_field(name="<@1236584616215711835>stop <ssh_command/Name>", value="Dừng một máy chủ.", inline=False)
    embed.add_field(name="<@1236584616215711835>regen-ssh <ssh_command/Name>", value="Tạo lại SSH credential", inline=False)
    embed.add_field(name="<@1236584616215711835>restart <ssh_command/Name>", value="Dừng máy chủ.", inline=False)
    embed.add_field(name="<@1236584616215711835>list", value="Liệt kê tất cả các máy chủ của bạn", inline=False)
    embed.add_field(name="<@1236584616215711835>ping", value="Kiểm tra ping của bot.", inline=False)
    embed.add_field(name="<@1236584616215711835>port-http", value="Chuyển tiếp một trang web http.", inline=False)
    embed.add_field(name="<@1236584616215711835>port-add", value="Chuyển tiếp một cổng.", inline=False)
    await interaction.response.send_message(embed=embed)
